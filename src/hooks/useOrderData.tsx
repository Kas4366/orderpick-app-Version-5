...
export const useOrderData = () => {
  ...
  const handleCustomerSearch = async (searchTerm: string) => {
    ...
    try {
      ...
      if (foundOrder) {
        ...
        // Mark all items in the group as completed when found via search
  ...
  const handleOrderComplete = async (order: Order, employeeName?: string) => {
    try {
      ...
      // Write packer info to Google Sheets via webhook if rowIndex exists
      if (order.rowIndex && employeeName) {
        try {
          ...
          if (result.success) {
            console.log(`✅ Updated packer info for order ${order.orderNumber} in Google Sheets`);
          } else {
            console.warn(`⚠️ Failed to update packer info: ${result.message}`);
          }
        } catch (error) {
          console.error('❌ Error sending packer info to webhook:', error);
        }
      }
    } catch (error) {
